...
@msig trait producer_t_i_producer_producer_TestApi {
   ...
   */
  def check_concrete_output(write_port: ISZ[Base_Types.Bits] => B): Unit = {
  ...
  def get_write_port(): Option[Base_Types.Bits] = {
    val value: Option[Base_Types.Bits] = get_write_port_payload() match {
      ...
      case Some(v) => halt(s"Unexpected payload on port write_port.  Expecting 'Base_Types.Bits_Payload' but received ${v}")
  ...
  def get_write_port_payload(): Option[Base_Types.Bits_Payload] = {
    return Art.observeOutInfrastructurePort(Arch.top_impl_Instance_producer_producer.initialization_api.write_port_Id).asInstanceOf[Option[Base_Types.Bits_Payload]]
  }
