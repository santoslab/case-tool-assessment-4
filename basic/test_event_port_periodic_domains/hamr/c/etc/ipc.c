...
Unit base_SharedMemory_receive(STACK_FRAME Z port, MBox2_43CC67 out) {
...
Unit base_SharedMemory_receiveAsync(STACK_FRAME Z port, MBox2_37E193 out) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit base_SharedMemory_send(STACK_FRAME Z appPortId, Z componentPortId, art_DataContent d) {
...
B base_SharedMemory_sendAsync(STACK_FRAME Z appPortId, Z componentPortId, art_DataContent d) {
    int sid = semget((key_t) componentPortId, 1, 0666);
